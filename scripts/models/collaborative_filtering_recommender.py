...
class CollabNNInference(nn.Module):
    def __init__(
        self,
        pretrained_user_factors=None,
        pretrained_game_factors=None,
        idx_to_app_id=None,
        app_id_to_idx=None,
        reference_dataset=None,
        binary_classification=False,
    ):
        super(CollabNNInference, self).__init__()
        self.user_factors = pretrained_user_factors
        self.game_factors = pretrained_game_factors
        self.hidden = nn.Sequential(
            nn.Linear(
                in_features=pretrained_user_factors.shape[1]
                + pretrained_game_factors.shape[1],
                out_features=16,
            ),
            nn.ReLU(),
            nn.Linear(in_features=16, out_features=64),
